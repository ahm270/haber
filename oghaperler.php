...
<div class="container-fluid">
	<div class="row">
		<div class="col-12">
				...
				<div class="form-row">
					<div class="col-md-6 mb-3">
						...
						<select class="custom-select mr-sm-2" name="Turu" required>
							...
							$ogn = $_GET["ogrencino"];
							include("vt.php");
							$sql = mysql_query("select * from  ogrenci where OGRENCI_NO = $ogn " );
							while($dizi = mysql_fetch_array($sql)) {
								echo'<input type="hidden" class="form-control" name="ogno" placeholder="haper" value="'.$dizi["OGRENCI_NO"]. '" required>';
			...
			on o.OGRENCI_NO=h.OGRENCI_NO inner join turu t on t.TURU_ID=h.TURU_ID where h.TC_NO is NULL and h.OGRENCI_NO = $ogn ");
				while($dizi1 = mysql_fetch_array($sql1))
				{
					echo '<tbody>
						<tr>
						
							<td><a href="?ogrencino='.$ogn.'&sil='.$dizi["HABER_ID"].'">sil</a></td>
							<td>'.$dizi1["unvan"].'</td>
...
if(isset($_POST["haberekle"])){
		...
		if(in_array(strtolower($_FILES['resim']['type']),$uzanti)){ 
		    ...
		    $sql = mysql_query("insert into haber (HABER_ADI,unvan,OGRENCI_NO, TURU_ID,RESIM_ID)
			values ( '$haper','$unvan',$ogno,$Turu,'$db')");}
		
		
		echo '<meta http-equiv="refresh" content="0; URL=oghaperler.php?ogrencino='.$ogn.'" />';
